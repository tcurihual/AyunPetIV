...
export default function PublicationDetail() {
  ...
  const handleSubmitReport = async (description: string) => {
    try {
      ...
      alert(`Reporte de ${reportType === "comment" ? "comentario" : "publicación"} enviado correctamente`)
    } catch (error) {
      ...
      <Animated.View style={styles.container} sharedTransitionTag={`pet-card-${pet.id}`}>
        <ScrollView showsVerticalScrollIndicator={false}>
          <View style={styles.infoContainer}>
            ...
            <View style={styles.infoContainer}>
              <Animated.Text style={styles.petName} sharedTransitionTag={`pet-name-${pet.id}`}>
                Nombre: {pet.name}
              </Animated.Text>
                ...
                {comments.length > 0 ? (
                  <View style={styles.commentsList}>
                    {comments.map((comment) => (
                      <CommentCard key={comment.id} comment={comment} onReport={handleReport} />
                    ))}
                  </View>
                ) : (
                  <View style={styles.commentsPlaceholder}>
                    <Text style={styles.commentsPlaceholderText}>Sé el primero en comentar...</Text>
                  </View>
                )}
              </View>
              ...
              <View style={styles.buttonContainer}>
                ...
                <TouchableOpacity style={styles.reportButton} onPress={handleReportPublication}>
